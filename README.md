...
Запустите сервер jupyter в браузере по умолчанию:  
```shell
jupyter notebook
``` 
...
python -m src.query
``` 
Либо через через OpenAPI в браузере по адресу http://localhost:8000
Там можно не только ознакомиться с описанием сервиса, но и отправлять тестовые
