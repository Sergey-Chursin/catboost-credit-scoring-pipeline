...
(замените project_ml_img на удобное вам).
```shell
docker build -t project_ml_img .
```

4. Запуск контейнера в интерактивном режиме с доступом к терминалу для выполнения скриптов вручную

```shell
docker run --rm -it project_ml_img bash
```
...
7. Запуск Jupyter notebook сервера

Перед запуском убедитесь, что Docker Desktop включён и работает.
```shell
docker run --rm -it -p 8888:8888 project_ml_img jupyter notebook --ip=0.0.0.0 --no-browser --allow-root
``` 
...
8. Закрытие контейнера с Jupyter notebook.
...
Функции mean_value_frequency_feature_pipeline и definite_value_proportion_features_pipeline генерирующие основную часть признаков почти избавлены от внутренних зависимостей и управляются через списки и словари задаваемые из конфигурационного файла config.py. Это позволяет гибко настраивать и изменять состав признаков без вмешательства в основной код.
...
По умолчанию функция реализует 5-кратную стратифицированную кросс-валидацию CatBoost, вычисляет метрику ROC AUC на обучающем,  
...
Поддерживаемые форматы: .pq, .parquet, .csv. Количество форматов можно легко расширить так как выбор функции загрузки организован
через mapping.
Опционально можно: 
...
В остальных случаях функция сверяется со списком метрик, требующих предсказанных меток классов, и запускает инференс нужного типа — предсказание меток классов  
либо вероятностей.
...
активируется в режиме  логирования Debug и выводит статистки оперативной памяти на входе декарируемой функции, перед выполнением её логики и
на выходе функции для сравнения нугрузки на RAM.
...
принудительно сжимает кучу (heap) аллокатора на Linux с glibc (ptmalloc2) — например, внутри Docker-контейнеров, 
безопасна для запуска на других платформах, но heap trimming будет недоступен (выводится соответствующее debug-сообщение). 
